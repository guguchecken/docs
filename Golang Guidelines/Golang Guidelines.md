...
- 【推荐】在短函数中应该使用命名返回值，以提高代码的简洁与清晰；返回值为slice或map的函数，出于性能考虑，应该在函数体内部进行初始化；但是在长函数中，不应该使用命名返回值。
...
- 【推荐】对于需要修改原对象或内存占用比较大的对象，应返回指针；对于只读或内存占用较小的对象，应返回值，尽量减少内存逃逸。
...
5. 插件配置
   ...
   vim-go: 需在vim中执行 `:GoInstallBinaries`，出现 `vim-go: installing finished!` 安装成功提示即可